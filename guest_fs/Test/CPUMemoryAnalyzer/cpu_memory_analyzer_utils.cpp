...
void print_proc_info(const proc_t &proc, ostream& out)
{
    out << "tid: " << setw(7) << proc.tid
         << setw(10)
         << "pid: " << setw(7) << proc.tgid
         << setw(20)
         << "vm_memory(KiB): " << setw(9) << proc.vm_size
         << "cmd: " << proc.cmd
         << endl;
}

void print_procs(const vector<proc_t> &procs, ostream& out)
{
    for (vector<proc_t>::const_iterator proc_it = procs.begin(); proc_it != procs.end(); ++proc_it) {
        print_proc_info(*proc_it, out);
    }
}
